...
def makeMetadata(documentRoot):
    ...
    meiHead = documentRoot.find(f'.//{MEI_NS}meiHead')
    if meiHead is not None:
        # title, subtitle, and movement name
        meta = metaSetTitle(meiHead, meta)
        # composer
        meta = metaSetComposer(meiHead, meta)
        # date
        meta = metaSetDate(meiHead, meta)
...
def metaSetTitle(work, meta):
    ...
    '''
    # title, subtitle, and movement name
    for title in work.findall(f'./{MEI_NS}fileDesc/{MEI_NS}titleStmt/{MEI_NS}title'):
...
def metaSetComposer(work, meta):
    ...
    '''
    composers = []
    for persName in work.findall(f'./{MEI_NS}fileDesc/{MEI_NS}titleStmt/{MEI_NS}respStmt/{MEI_NS}persName'):
        if persName.get('role') == 'composer' and persName.text:
            composers.append(persName.text)
    for persName in work.findall(f'./{MEI_NS}fileDesc/{MEI_NS}titleStmt/{MEI_NS}respStmt/{MEI_NS}name'):
        if persName.get('role') == 'composer' and persName.text:
            composers.append(persName.text)
    for composer in work.findall(f'./{MEI_NS}fileDesc/{MEI_NS}titleStmt/{MEI_NS}composer'):
        if composer.text:
            composers.append(composer.text)
        else:
            persName = composer.find(f'.{MEI_NS}fileDesc/{MEI_NS}source/{MEI_NS}persName')
            if persName.text:
                composers.append(persName.text)
    if len(composers) == 1:
...
def _correctMRestDurs_2 (staves, targetLength):
    ''' when the mei is generated from musicxml breve rests may not be handled correctly: they get a 1 value.
    Therefore a double check is needed: if a voice only has one rest it needs to get the target length'''
    for eachMeasure in staves.values():
        for eachVoice in eachMeasure:
            if not isinstance(eachVoice, stream.Stream):
                continue
            
            if len(eachVoice.elements) == 1 and  isinstance (eachVoice.elements[0], note.Rest):
                eachVoice.elements[0].duration = duration.Duration(targetLength)
...
def measureFromElement(elem, backupNum, expectedNs, slurBundle=None, activeMeter=None):
    ...
    **Contained Elements not Implemented:**
    ...
    for eachElem in elem.iterfind('*'):
        ...
        elif dirTag == eachElem.tag:
            if eachElem.attrib["label"]=="words": 
                expressionFromElement(eachElem, staves, activeMeter) # words 
                print()
    ...
    _correctMRestDurs_2(staves, maxBarDuration)
...
def expressionFromElement(elem, staffs, activeMeter):
    
    if activeMeter == None:
        print ("Cannot convert expression: no active meter ")
    
    ''' get staff and offset infos'''
    tstsamp = elem.get('tstamp')
    staff = elem.get('staff')
    
    ''' check to which offset it should be associated '''    
    offset = (float(tstsamp)-activeMeter.beat) *activeMeter.beatLengthToQuarterLengthRatio 
    
    
    ''' crate dir according to its type '''
    if textStripValid(elem):
        wordText = elem.text.strip()
    else:
        wordText = ''
    te = expressions.TextExpression(wordText)
    
    
    for element in staffs[staff].flat.elements:
        if hasattr(element, "offset") and hasattr(element, "duration"): 
            if element.offset == offset: # exact match
                staffs[staff].insert(element.offset, te)  
                break
            
            # if no exact match get the element whose endtime includes the offset
            if (element.offset + element.duration.quarterLength) > offset:
                staffs[staff].insert(element.offset, te)  
                break
...
def getPartInformation (scoreElem, allPartNs):
    # CGN: used to get additional part information i.e partName, partAbbreviation, id derived from score deff
    # Returns partsInformationDictionary 
    partsInformationDictionary = {}
    
    for partN in allPartNs:
        partInformationDictionary = {} # info for one single part
        xpathQuery = f'.//{MEI_NS}staffDef[@n="{partN}"]'
    
        for staffDef in scoreElem.findall(xpathQuery):
            # map 'n' to 'id'
            partInformationDictionary["id"] = staffDef.get(_XMLID) 
            
            # map 'label' to 'partName'
            if staffDef.get('label'): partInformationDictionary["partName"] = staffDef.get('label')
            
            # map 'label.abbr' to 'partAbbreviation'
            if staffDef.get('label.abbr'): partInformationDictionary["partAbbreviation"] = staffDef.get('label.abbr')
            
        partsInformationDictionary[partN] = partInformationDictionary
            
    
    return partsInformationDictionary
...
def scoreFromElement(elem, slurBundle):
    ...
    **Contained Elements not Implemented:**
    ...
    # CGN: get additional part information
    partInformation = getPartInformation(elem, allPartNs)
    ...
    for i, eachN in enumerate(allPartNs):
        ...
        # CGN: add part information derived from score def
        for attribute, value in partInformation[eachN].items():
            setattr(theScore[i], attribute, value) 

...
class Converter:
    ...
    def parseURL(self, url, format=None, forceSource=False, number=None, **keywords): # @ReservedAssignment
        ...
        if not dst.exists() or forceSource==True:
            try:
...
def parseURL(url, number=None, format=None, forceSource=False, **keywords): # @ReservedAssignment
    ...
    '''
    v = Converter()
    v.parseURL(url, format, forceSource, number, **keywords)
    return v.stream
